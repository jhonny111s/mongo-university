...
/*      var categories = [];
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
       ...
       /*  var pageItem = this.createDummyItem();
        ...
        var collection = this.db.collection( 'item' );
        var cursor = collection.find(query).skip(page*itemsPerPage).limit(itemsPerPage).sort({_id:1});
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        var cursor =  collection.aggregate([
            ...
            .skip(page*itemsPerPage).limit(itemsPerPage).sort({_id:1});
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
       ...
       /*  var reviewDoc = {
        ...
        var collection = this.db.collection( 'item' );
        collection.findOneAndUpdate({'_id': itemId}, {$addToSet: {reviews: reviewDoc}}, function(error, item) {
            assert.equal(error, null);
            callback(item);
        });
